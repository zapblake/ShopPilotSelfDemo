...
import type { WidgetPreviewConfig, PageContext, StoreContext, DemoFlags } from "./types";
...
export async function buildWidgetConfig(
  ...
  const renderedPage = job.renderedPages.find(
    (rp) => rp.previewPath === currentPath
  ) ?? job.renderedPages.find(
    (rp) => rp.previewPath === "/" || rp.previewPath === ""
  ) ?? job.renderedPages[0];
  ...
  const storeName =
    ...
    job.normalizedDomain.replace(/\.\w+$/, "").replace(/(^|\.)(\w)/g, (_m, _p, c: string) => ` ${c.toUpperCase()}`).trim();
  ...
  const sampleProducts = discoveredPages
    .filter((p) => p.pageType === "PRODUCT" || p.pageTypeCandidate?.toLowerCase().includes("product"))
    .slice(0, 3)
    .map((p) => ({
      title: p.title ?? p.url,
      url: p.url,
